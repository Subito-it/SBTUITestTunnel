...
class StubTests: XCTestCase {
    ...
    func testStubResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", responseHeaders: ["Content-Type": "application.*", "Server": "istio-envoy|nginx"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
    ...
    func testStubRequestAndResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", requestHeaders: ["Accept": "gzip.*"], responseHeaders: ["Content-Type": "application.*", "Server": "istio-envoy|nginx"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
