...
class StubTests: XCTestCase {
    ...
    func testStubResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", responseHeaders: ["Content-Type": "application.*", "Server": "gunicorn"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
    ...
    func testStubRequestAndResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", requestHeaders: ["Accept": "gzip.*", "Accept-Encoding": "gzip"], responseHeaders: ["Content-Type": "application.*", "Server": "gunicorn"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
