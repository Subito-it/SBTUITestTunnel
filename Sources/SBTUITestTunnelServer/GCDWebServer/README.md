...
We embed the code of GCDWebServer which is no longer maintaned to overcome issues due to the very old deployment target that is set in the original podspec.

To avoid symbol clashing we did rename `GCDweb` -> `SBTWeb` which should allow to use the tunnel in projects that are already using the original version of GCDWebServer.
