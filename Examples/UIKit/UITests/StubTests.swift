...
class StubTests: XCTestCase {
    ...
    func testStubResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", responseHeaders: ["Content-Type": "application.*", "Server": "istio-envoy|nginx|cloudflare"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
    ...
    func testStubRequestAndResponseHeaders() {
        ...
        _ = {
            let match = SBTRequestMatch(url: "postman-echo.com", requestHeaders: ["Accept": "gzip.*"], responseHeaders: ["Content-Type": "application.*", "Server": "istio-envoy|nginx|cloudflare"])
            let stubId = self.app.stubRequests(matching: match, response: SBTStubResponse(response: ["stubbed": 1]))!
    ...
    func testStubActiveCount() {
        ...
        XCTAssertEqual(stubs5.first(where: { $0.match == match2 })?.response.activeIterations, 0)
    }
    ...
    func testStubActiveCountRemoving() {
        ...
        XCTAssertNil(stubs3.first(where: { $0.match == match1 }))
        XCTAssertNil(stubs3.first(where: { $0.match == match2 }))
    }
    ...
    func testStubActiveCountRemovingAll() {
        ...
        XCTAssertNil(stubs2.first(where: { $0.match == match1 }))
        XCTAssertNil(stubs2.first(where: { $0.match == match2 }))
    }
